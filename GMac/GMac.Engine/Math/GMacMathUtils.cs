...
namespace GMac.Engine.Math
{
    public static class GMacMathUtils
    {
        ...
        private static void SetMaxVSpaceDimension()
        {
            ...
            try
            {
                var value = GMacEngineUtils.Settings["maxVSpaceDimension"];

                MaxVSpaceDimension = 
                    int.TryParse(value, out var i) 
                    ? System.Math.Max(defaultMaxVSpaceDimension, System.Math.Min(i, GMacCompilerFeatures.MaxFrameDimension)) 
