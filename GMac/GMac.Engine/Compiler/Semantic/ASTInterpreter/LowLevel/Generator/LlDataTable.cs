...
namespace GMac.Engine.Compiler.Semantic.ASTInterpreter.LowLevel.Generator
{
    ...
    internal sealed class LlDataTable
    {
        ...
        private ValuePrimitive<MathematicaScalar> ReadRHSPrimitiveValue_ExistingOnly(LanguageValueAccess valueAccess)
        {
            ...
            if (_hlDictionary.TryGetValue(hlName, out var dataInfo))
            {
        ...
        public void WriteLhsStructureValue(LanguageValueAccess valueAccess, ValueStructureSparse structValue)
        {
            ...
            foreach (var dataMember in structure.DataMembers)
            {
                if (!structValue.TryGetValue(dataMember.ObjectName, out var dataMemberValue)) 
                    continue;
        ...
        {
            return (_llDictionary.TryGetValue(llItemName, out var llItem) && llItem.IsVariableInput);
        }
        ...
        {
            return (_llDictionary.TryGetValue(llItemName, out var llItem) && llItem.IsOutput);
        }

        public bool IsVariableTemp(string llItemName)
        {
            return (_llDictionary.TryGetValue(llItemName, out var llItem) && llItem.IsVariableTemp);
        }
