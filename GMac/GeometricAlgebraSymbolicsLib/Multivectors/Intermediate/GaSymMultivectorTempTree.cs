...
namespace GeometricAlgebraSymbolicsLib.Multivectors.Intermediate
{
    public sealed class GaSymMultivectorTempTree : IGaSymMultivectorTemp
    {
        ...
        private readonly GaBinaryTreeInternalNode<List<Expr>> _termsTree;
        ...
        public Expr this[int id]
        {
            get
            {
                _termsTree.TryGetLeafValue((ulong) id, out var coef);
        ...
        {
            _termsTree = new GaBinaryTreeInternalNode<List<Expr>>(
                0,
                gaSpaceDim.ToVSpaceDimension()
            );
        }
