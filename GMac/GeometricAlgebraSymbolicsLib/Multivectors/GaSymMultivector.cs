...
namespace GeometricAlgebraSymbolicsLib.Multivectors
{
    public sealed class GaSymMultivector : IGaSymMultivector, ISymbolicVector
    {
        ...
        public static GaSymMultivector CreateCopy(GaBinaryTreeInternalNode<Expr> termsTree)
        {
        ...
        internal GaBinaryTreeInternalNode<Expr> TermsTree { get; }
        ...
        public int Size => GaSpaceDimension;
        ...
        public IEnumerable<MathematicaScalar> BasisBladeScalars 
            => TermsTree
                .LeafValuePairs
                .Select(pair => pair.Value?.ToMathematicaScalar() ?? GaSymbolicsUtils.Constants.Zero);

        public IEnumerable<Expr> BasisBladeExprScalars 
            => TermsTree
                .LeafValuePairs
                .Select(pair => pair.Value ?? Expr.INT_ZERO);

        public IEnumerable<MathematicaScalar> NonZeroBasisBladeScalars 
            => TermsTree
            .LeafValuePairs
            .Where(pair => !pair.Value.IsNullOrZero())
            .Select(pair => pair.Value.ToMathematicaScalar());
        ...
        private GaSymMultivector(int gaSpaceDim)
        {
            ...
            TermsTree = new GaBinaryTreeInternalNode<Expr>(0, VSpaceDimension);
        }
        ...
        public bool IsScalar()
        {
            return TermsTree.HasNoChildNodes ||
                   TermsTree.LeafValuePairs.All(p => p.Key == 0 || p.Value.IsNullOrZero());
        }

        public bool IsZero()
        {
            return TermsTree.HasNoChildNodes || 
                   TermsTree.LeafValues.All(v => v.IsNullOrZero());
        }

        public bool IsNearNumericZero(double epsilon)
        {
            return TermsTree.HasNoChildNodes ||
                   TermsTree.LeafValues.All(v => v.IsNullOrNearNumericZero(epsilon));
        }
