...
{
    public sealed class GMacHashTable2D<T> : IEnumerable<Tuple<int, int, T>>
    {
        ...
        public T this[int key1, int key2]
        {
            ...
            {
                T value;
                _dictionary.TryGetValue((ulong)key1 + ((ulong)key2 << 32), out value);
        ...
        public GMacHashTable2D(Func<T, bool> isDefaultValueFunc)
        {
        ...
        public GMacHashTable2D<T> Remove(int key1, int key2)
        {
