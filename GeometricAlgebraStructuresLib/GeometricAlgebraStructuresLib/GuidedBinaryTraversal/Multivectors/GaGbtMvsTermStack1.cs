...
namespace GeometricAlgebraStructuresLib.GuidedBinaryTraversal.Multivectors
{
    ...
    {
        public static GaGbtMvsTermStack1<T> Create(int capacity, GaMvsTerm<T> termStorage)
        {
            return new GaGbtMvsTermStack1<T>(capacity, termStorage);
        }


        public GaMvsTerm<T> TermStorage { get; }

        public IGaMultivectorStorage<T> MultivectorStorage 
            => TermStorage;

        public T TosValue { get; private set; }


        private GaGbtMvsTermStack1(int capacity, GaMvsTerm<T> termStorage)
