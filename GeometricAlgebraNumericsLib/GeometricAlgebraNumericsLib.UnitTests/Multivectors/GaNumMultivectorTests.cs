...
{
    [TestFixture]
    public sealed class GaNumMultivectorTests
    {
        ...
        private GaNumMultivector CreateSource(string factoryKind)
        {
            if (factoryKind == "dar")
                return _termsArray1.CreateDarMultivector(VSpaceDimension);

            if (factoryKind == "dgr")
                return _termsArray1.CreateDgrMultivector(VSpaceDimension);

            if (factoryKind == "sar")
                return _termsArray1.CreateSarMultivector(VSpaceDimension);

            if (factoryKind == "sgr")
                return _termsArray1.CreateSgrMultivector(VSpaceDimension);
        ...
        [Test, Combinatorial]
        public void ConvertMultivectorToMultivector(
        ...
        [Test, Combinatorial]
        public void ReadMultivectorTermsById(
            [Values("dar", "dgr", "sar", "sgr")] string factoryKind,
            [Values("dar", "dgr", "sar", "sgr")] string multivectorKind
        )
        {
            ...
            var resultMultivector =
                CreateMultivector(factory, multivectorKind, "copy");

            for (var id = 0; id < GaSpaceDimension; id++)
            {
                Assert.IsTrue(
                    resultMultivector[id] == _referenceMultivector1[id]
                );
            }
        }

        [Test, Combinatorial]
        public void ReadMultivectorTermsByGradeIndex(
            [Values("dar", "dgr", "sar", "sgr")] string factoryKind,
            [Values("dar", "dgr", "sar", "sgr")] string multivectorKind
        )
        {
            ...
            for (var grade = 0; grade <= VSpaceDimension; grade++)
            {
                ...
                {
                    Assert.IsTrue(
                        resultMultivector[grade, index] == _referenceMultivector1[grade, index]
                    );
                }
            }
        }
