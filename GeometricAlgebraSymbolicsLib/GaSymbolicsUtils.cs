...
namespace GeometricAlgebraSymbolicsLib
{
    public static class GaSymbolicsUtils
    {
        ...
        public static GaBinaryTree<int> ComputeBasisBladesSignatures(this IReadOnlyList<int> basisVectorsSignaturesList)
        {
            var vSpaceDim = basisVectorsSignaturesList.Count;
            var bbsList = new GaBinaryTree<int>(vSpaceDim);
        ...
        public static GaBinaryTree<double> ComputeBasisBladesSignatures(this IReadOnlyList<double> basisVectorsSignaturesList)
        {
            var vSpaceDim = basisVectorsSignaturesList.Count;
            var bbsList = new GaBinaryTree<double>(vSpaceDim);
        ...
        public static GaBinaryTree<MathematicaScalar> ToSymbolicTree(this GaBinaryTree<int> rootTreeNode)
        {
        ...
        public static GaBinaryTree<MathematicaScalar> ToSymbolicTree(this GaBinaryTree<double> rootTreeNode)
        {
            return rootTreeNode?.MapLeafValues(
                i => MathematicaScalar.Create(Cas, i)
            );
        ...
        public static GaSymMultivector ToSymbolic(this GaNumMultivector mv)
        {
            var resultMv = GaSymMultivector.CreateZero(mv.GaSpaceDimension);

            foreach (var term in mv.NonZeroTerms)
                resultMv.SetTermCoef(term.Key, term.Value.ToExpr());

            return resultMv;
        }

        public static GaNumMultivector ToNumeric(this GaSymMultivector mv)
        {
            var resultMv = GaNumMultivector.CreateZero(mv.GaSpaceDimension);

            foreach (var term in mv.NonZeroExprTerms)
                resultMv.SetTermCoef(term.Key, term.Value.ToNumber());
        ...
        public static double ToNumber(this Expr value)
        {
            ...
            var textValue =
                GaSymbolicsUtils.Cas.Connection.EvaluateToOutputForm(Mfs.CForm[Mfs.N[value]]);

            double doubleValue;
            return double.TryParse(textValue, out doubleValue)
                ? doubleValue : 0.0d;
        }
